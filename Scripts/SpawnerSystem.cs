...
public class SpawnerSystem : JobComponentSystem
{
    ...
    struct SpawnJob : IJobProccessComponentDataWithEntity<Spawner, LocalToWorld>
    {
            ...
            [ReadOnly] ref LocalToWorld location)
        {
            for (int x = 0; x < spawner.Erows; x++)
            {
                for (int z = 0; z < spawner.Ecols; z++)
                {
                    ...
                    var pos = math.transform(location.Value, new float3(x, noise.cnoise(
                        new float2(x, z) * 0.21f), z));
